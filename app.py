...
for p in prompts:
    with st.expander(f"{p[1]}"):
        ...
        if edit:
            ...
            edited_title = st.text_input("Title", value=prompt_to_edit.title, key=f"edit_title_{p[0]}")
            edited_prompt = st.text_area("Prompt", value=prompt_to_edit.prompt, key=f"edit_prompt_{p[0]}")
            if st.button("Save", key=f"save_edit_{p[0]}"):
                cur.execute("UPDATE prompts SET title = %s, prompt = %s WHERE id = %s", (edited_title, edited_prompt, p[0]))
                con.commit()
                st.success(f"Prompt {p[0]} updated successfully!")
                st.experimental_rerun()
# import os
# from dataclasses import dataclass, field
# import streamlit as st
